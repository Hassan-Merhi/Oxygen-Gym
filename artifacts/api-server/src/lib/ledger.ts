...
/** Reconstruct physical Cash from authoritative source rows using each transaction's locked FX. */
export async function getCurrentBalance(executor: DbExecutor = db): Promise<{ balanceUsd: number; balanceCdf: number }> {
  const result = await executor.execute(sql`
    ...
    opening_adjustments AS (
      SELECT
        COALESCE(SUM(CASE WHEN cl.direction = 'in' THEN
          COALESCE(cl.amount_usd, CASE
            ...
            ELSE 0 END)
        ELSE -COALESCE(cl.amount_usd, CASE
            WHEN cl.currency = 'USD' THEN cl.amount
            WHEN COALESCE(cl.exchange_rate, 0) > 0 THEN cl.amount / cl.exchange_rate
            ELSE 0 END) END), 0) AS usd,
        COALESCE(SUM(CASE WHEN cl.direction = 'in' THEN
          COALESCE(cl.amount_cdf, CASE
            WHEN cl.currency = 'CDF' THEN cl.amount
            WHEN COALESCE(cl.exchange_rate, 0) > 0 THEN cl.amount * cl.exchange_rate
            ELSE 0 END)
        ELSE -COALESCE(cl.amount_cdf, CASE
            WHEN cl.currency = 'CDF' THEN cl.amount
            WHEN COALESCE(cl.exchange_rate, 0) > 0 THEN cl.amount * cl.exchange_rate
            ELSE 0 END) END), 0) AS cdf
      FROM cash_ledger cl
