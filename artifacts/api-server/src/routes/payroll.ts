...
router.get("/:id", async (req: Request, res: Response) => {
  const id = Number(contractParams(req, ApiContracts.GetPayrollParams).id);
...
router.patch("/:id/pay", async (req: Request, res: Response) => {
  const id = Number(contractParams(req, ApiContracts.MarkPayrollPaidParams).id);
  const [record] = await db.select().from(payrollTable).where(eq(payrollTable.id, id));
  if (!record) { res.status(404).json({ error: "Payroll record not found" }); return; }
...
router.patch("/:id/cancel", async (req: Request, res: Response) => {
  ...
  const id = Number(contractParams(req, ApiContracts.CancelPayrollParams).id);
