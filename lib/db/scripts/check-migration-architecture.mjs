...
for (const file of await walk(apiSourceRoot)) {
  ...
  if (/UPDATE\s+products\s+SET\s+quantity\s*=\s*c\.purchased/i.test(source)) {
    fail(`runtime source still contains the one-time inventory repair: ${path.relative(repoRoot, file)}`);
  }
}

const paymentsRouter = await text("artifacts/api-server/src/domains/accounting/payments-router.ts");
if (/await\s+cashCleanup\s*\(/.test(paymentsRouter) || /\bcashCleanup\s*[,}]/.test(paymentsRouter)) {
  fail("payments router still exposes the historical cash cleanup repair");
}

const auditRoute = await text("artifacts/api-server/src/routes/audit.ts");
if (!/router\.post\("\/fix\/inventory"[\s\S]*?status\(410\)/.test(auditRoute)) {
  fail("inventory repair compatibility endpoint is not fail-closed (410)");
}

const renderBlueprint = await text("render.yaml");
if (!/buildCommand:.*pnpm run db:migrate/.test(renderBlueprint)) {
  fail("Render build command does not apply versioned migrations explicitly");
}
if (/startCommand:.*db:migrate/.test(renderBlueprint)) {
  fail("Render start command must not run database migrations");
}

const rootPackage = JSON.parse(await text("package.json"));
if (rootPackage.scripts?.["db:migrate"] !== "pnpm --filter @workspace/db run migrate") {
  fail("root db:migrate command is missing or points at an unexpected command");
}

const transactionFacade = await text("artifacts/api-server/src/shared/db/transaction.ts");
if (!transactionFacade.includes('from "@workspace/db"')) {
  fail("API transaction facade is not backed by the canonical @workspace/db transaction helper");
}

const transactionalServices = [
  "artifacts/api-server/src/domains/accounting/payment-service.ts",
  "artifacts/api-server/src/domains/accounting/voucher-service.ts",
  "artifacts/api-server/src/domains/members/command-service.ts",
  "artifacts/api-server/src/domains/payroll/service.ts",
  "artifacts/api-server/src/domains/sales/service.ts",
  "artifacts/api-server/src/domains/inventory/service.ts",
  "artifacts/api-server/src/domains/inventory/supplier-credit-service.ts",
];
for (const relativePath of transactionalServices) {
  const source = await text(relativePath);
  if (!source.includes("withTransaction(")) {
    fail(`${relativePath} no longer uses the standard transaction wrapper for multi-step writes`);
  }
}

const currencyHelper = await text("artifacts/api-server/src/shared/accounting/currency.ts");
if (!currencyHelper.includes("../../repositories/settings")) {
  fail("shared currency helper bypasses the settings repository");
}

const settingsRepository = await text("artifacts/api-server/src/repositories/settings.ts");
if (!settingsRepository.includes("DbExecutor") || !settingsRepository.includes("settingsTable")) {
  fail("settings repository is missing executor-aware database query helpers");
}
