...
if (!accounting.includes('"Supplier Payables"')) fail("Supplier Payables is missing from accounting account typing");
...
for (const required of ["appendLedgerEntry", "postDoubleEntry", "reverseEntries", "exchangeRate: rate", "toUsdCdf", "...converted"]) {
...
try {
  ...
  for (const column of ["exchange_rate", "amount_usd", "amount_cdf", "account", "payment_id"]) {
